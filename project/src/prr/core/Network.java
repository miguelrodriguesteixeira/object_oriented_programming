...
public class Network implements Serializable {
     ...
     */
    public void sendTextCommunication(Terminal from, String toKey, String msg) throws UnknownTerminalKeyException, DestinationOffException, UnknownClientKeyException {
        Terminal to = getTerminal(toKey);
        if (to.getOwner().getAllMadeCommunications().size() >= 100)
            throw new UnknownClientKeyException(to.getOwner().getClientKey());
    ...
    public boolean removeClientByTaxId(int taxId) {
        for (Client c : _clients.values())
            if (c.getTaxNumber() == taxId) {
                _clients.remove(c.getClientKey());
                return true;
            }
        return false;
    }

    public String getMostExpensiveVoiceComm() {
        ArrayList<Communication> communications = new ArrayList<>(_communications.values());
        communications.removeIf(communication -> !(communication instanceof VoiceCommunication));
        if (communications.size() == 0)
            return null;
        Communication max = communications.get(0);
        for (Communication c : communications) {
            if (c.getCost() > max.getCost())
                max = c;
            else if (c.getCost() == max.getCost())
                if (c.getId()> max.getId())
                    max = c;
        }
        return max.getId() + "| " +  max.getFrom().getId();
    }
